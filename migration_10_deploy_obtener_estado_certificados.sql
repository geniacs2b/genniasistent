-- ─────────────────────────────────────────────────────────────────────────────
-- Migration 10 — Desplegar RPC obtener_estado_certificados_evento
-- Contexto: la función no estaba desplegada en producción.
-- La vista /app/participantes depende de esta función para mostrar el estado
-- de certificación enriquecido (sesiones, asistencias, habilitaciones, envíos).
-- ─────────────────────────────────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION public.obtener_estado_certificados_evento(
...
LANGUAGE plpgsql
STABLE
SECURITY INVOKER   -- respeta las políticas RLS del usuario llamante
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN COALESCE(hab.habilitado, false)
        OR (COALESCE(ac.total_asistencias, 0) >= ins.min_sesiones_certificado) THEN 'Autorizado'
      ELSE 'No enviado'
    END,
    CASE
      ...
      WHEN (COALESCE(ac.total_asistencias, 0) >= ins.min_sesiones_certificado)
        OR COALESCE(hab.habilitado, false)   THEN 'enviar'
      ELSE NULL
    END,
    ins.ev_titulo,
    ins.total_sesiones,
    COALESCE(ac.total_asistencias, 0)::bigint
  FROM inscritos ins
  LEFT JOIN asistencias_count ac
    ON  ins.v_persona_id = ac.persona_id AND ins.v_ev_id = ac.evento_id
  LEFT JOIN habilitaciones    hab
    ON  ins.v_persona_id = hab.persona_id AND ins.v_ev_id = hab.evento_id
  LEFT JOIN envios            env
    ON  ins.v_persona_id = env.persona_id
    AND ins.v_inscripcion_id = env.inscripcion_id
    AND ins.v_ev_id = env.evento_id;
END;
$$;

COMMENT ON FUNCTION public.obtener_estado_certificados_evento(uuid) IS
  'Devuelve estado de certificados por inscripción, respetando RLS del tenant.
   Parámetro opcional p_evento_id para filtrar por evento específico.';
