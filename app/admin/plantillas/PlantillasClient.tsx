...
export function PlantillasClient({ initialTemplates, events }: { initialTemplates: any[], events: any[] }) {
  ...
  const BUCKET = "certificados-base";
  ...
  const handleUpload = async () => {
    ...
    // Detectar dimensiones con Image
    const img = new Image();
    img.onload = async () => {
      ...
      if (dbErr) {
        toast({ title: "Error al guardar la plantilla", description: dbErr.message, variant: "destructive" });
        setUploading(false);
        return;
      }

      // 2. Asociar plantilla al evento
      ...
      setUploading(false);
      if (eventErr) {
        toast({ title: "Error al asociar la plantilla al evento", description: eventErr.message, variant: "destructive" });
      } else {
